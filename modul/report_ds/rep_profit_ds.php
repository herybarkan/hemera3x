...
<div class="row">
                        <div class="col-sm-12">
                           <div class="element-wrapper">
                              <h6 class="element-header">Profit Dropshipper</h6>
                              <div class="element-box">
                                 <h5 class="form-header">Profit Dropshipper periode <?php echo date('Y'); ?></h5>
                                 <div class="form-desc">Grafik Dropshipper dilihat dari view Dropshipper</a></div>
...
ON product_master.kd_product = trx_ds_detail.kd_prod 
JOIN trx_ds
ON trx_ds.kd_order = trx_ds_detail.kd_order WHERE trx_ds.kd_ds='$row_ruser2[kd_dropshiper]' AND trx_ds_detail.sts_wd= '0' AND trx_ds.sts_kirim='1'";
...
mysql_select_db($database_hemera, $hemera);
$query_ptotx = "SELECT
SUM((trx_ds_detail.hrg_satuan-product_master.harga_net)*trx_ds_detail.qty) AS profitx
FROM
product_master
JOIN trx_ds_detail
ON product_master.kd_product = trx_ds_detail.kd_prod 
JOIN trx_ds
ON trx_ds.kd_order = trx_ds_detail.kd_order WHERE trx_ds.kd_ds='$row_ruser3[kd_dropshiper]' AND trx_ds_detail.sts_wd= '1' AND trx_ds.sts_kirim='1'";
$ptotx = mysql_query($query_ptotx, $hemera) or die(mysql_error());
$row_ptotx = mysql_fetch_assoc($ptotx);
$totalRows_ptotx = mysql_num_rows($ptotx);	

		?>
		<?php
			if ($row_ptotx['profitx']=="") {$xprofitx="0";}
		elseif ($row_ptotx['profitx']!="") {$xprofitx = $row_ptotx['profitx'];}
		echo $xprofitx.", ";
		?>
		<?php } while ($row_ruser3 = mysql_fetch_assoc($ruser3)); ?>
		//0, 0, 106.4, 129.2, 144.0, 176.0, 135.6, 148.5, 216.4, 194.1, 95.6, 54.4
		]
