...
trx_ds.sts_bayar='1' AND trx_ds.sts_proses='1' AND trx_ds.sts_kirim='1' AND product_master.kd_suplier='$row_rcek_sup[kd_suplier]' 
$qsds $qtgl $qcustomer 
GROUP BY
trx_ds.kd_order
ORDER BY
...
<form class="form-inline justify-content-sm-end" id="fsearch" name="fsearch" method="POST" action="index.php?com=rep_delivery_sup&layout=full">
...
<a href="index.php?com=rep_delivery_sup&layout=full"><input name="bt_reload" type="button" class="btn btn-rounded btn-outline-danger" style="margin-left:10px;" value="Reload"/></a>
</form>
</div>
   <h6 class="element-header">Delivery Report
   ...
   <div class="element-box-tp">
      <div class="table-responsive">
         <table width="100%"  class="table table-padded wrap ">
            ...
            <tbody>
            <?php $tot_harga=0; $xqty=0; $xship=0; $xtp=0;?>
               <?php do { ?>
               <tr>
                   <td align="center">
                   <a href="javascript:void(0);" data-href="modul/supplier/detail_delivery_sup.php?kd_order=<?php echo $row_rorder['kd_order']; ?>" class="btn btn-success btn-sm text-white openPopup" style="width:100px;">Detail</a>
                    <br>
                   </td>
                   <td><?php
mysql_select_db($database_hemera, $hemera);
...
$totalRows_rqty = mysql_num_rows($rqty);
                   ...
                   <td><?php echo $row_rorder['tgl_kirim']; ?></td>
                ...
                <?php 
				$tot_harga+=$row_rorder['hrg_tot']; 
				$xqty+=$row_rqty['jqty'];
				$xship+=$row_rorder['hrg_shipping']; 
				$xtp+=$row_rorder['hrg_grand'];
				?>
                 <?php } while ($row_rorder = mysql_fetch_assoc($rorder)); ?>
               <tr>
                 <td colspan="5" align="left"><span style="font-weight:700; color:#F00;">TOTAL</span></td>
                 <td><span style="font-weight:700; color:#F00;">
				 <?php echo number_format($xqty,0,',','.'); ?></span></td>
                 <td class="text-center"><span style="font-weight:700; color:#F00;">
				 IDR. <?php echo number_format($tot_harga,0,',','.'); ?></span></td>
                 <td><span style="font-weight:700; color:#F00;">
				 IDR. <?php echo number_format($xship,0,',','.'); ?></span></td>
                 <td><span style="font-weight:700; color:#F00;">
				 IDR. <?php echo number_format($xtp,0,',','.'); ?></span></td>
               </tr>
            </tbody>
         </table>
